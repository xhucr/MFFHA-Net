...
# from Datasets.ISIC2017_ import ISIC2017_dataset
...
from Models.mffha_net import MFFHA_Net
...
def train(train_loader, model, criterion, scheduler, optimizer, args, epoch):
    ...
    for step, (x, y) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if criterion == "loss_C":
            dice_loss = soft_dice_loss2(output.clone(), target_soft.clone())
            ce_loss = CE_loss_F(output.clone(), target.clone())
            with torch.cuda.amp.autocast():
                loss = torch.nan_to_num(dice_loss + ce_loss)
...
def valid_isic(valid_loader, model, criterion, optimizer, args, epoch, best_score, val_acc_log):
    ...
    print('The ISIC Dice score: {dice: .4f}; '
          'The ISIC JC score: {jc: .4f}'.format(
           dice=isic_dice_mean, jc=isic_Jaccard_mean))
           
    with open(val_acc_log, 'a') as vlog_file:
        line = "{} | {dice: .4f} | {jc: .4f}".format(epoch, dice=isic_dice_mean, jc=isic_Jaccard_mean)
   ...
   # isic_assd = []
    ...
    for step, (name, img, lab) in tqdm(enumerate(test_loader), total=len(test_loader)):
       ...
       # isic_assd.append(isic_b_asd)
        ...
        isic_Jaccard.append(isic_b_Jaccard)
        isic_dc.append(isic_b_dc)
        # vis(image,target,output_dis,step+1)
...
def main(args, val_acc_log, test_acc_log):
   ...
   #  scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=20, T_mult=2, eta_min = 0.0001)
    ...
    #scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=50, T_mult=1, eta_min = 0.00001)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--id', default="MFFHA-Net",
                        help='MFFHA-Net')                                                   # model name
    ...
    parser.add_argument('--weight_decay', default=1e-10, type=float, help='weights regularizer')
    ...
    args.ckpt = os.path.join(args.ckpt, args.data, args.val_folder, args.id + "_{}_e".format(criterion))

    if not os.path.isdir(args.ckpt):
        os.makedirs(args.ckpt)
    logfile = os.path.join(args.ckpt,'{}_{}_{}_onlymap.txt'.format(args.val_folder, args.id, criterion))          # path of the training log

    sys.stdout = Logger(logfile)  
    
    val_acc_log = os.path.join(args.ckpt,'val_acc_{}_{}_{}_onlymap.txt'.format(args.val_folder, args.id, criterion))
    test_acc_log = os.path.join(args.ckpt,'test_acc_{}_{}_{}_onlymap.txt'.format(args.val_folder, args.id, criterion))
